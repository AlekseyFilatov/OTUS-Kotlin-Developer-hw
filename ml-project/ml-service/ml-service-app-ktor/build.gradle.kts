...
kotlin {
    ...
    targets.withType<org.jetbrains.kotlin.gradle.plugin.mpp.KotlinNativeTarget> {
        binaries {
            executable {
                entryPoint = "api.kotlinproject.app.ktor.main"
            }
        }
    }
...
//                implementation("io.ktor:ktor-server-double-receive:${libs.versions.ktor.get()}")
                ...
                implementation(project(":ml-service-app-common"))
...
/*tasks {
    ...
    val dockerDockerfileX64 by creating(Dockerfile::class) {
        ...
        group = "docker"
        from(Dockerfile.From("ubuntu:24.04").withPlatform("linux/amd64"))
        doFirst {
            copy {
                from(nativeFileX64)
        ...
        copyFile(nativeFileX64.name, "/")
        copyFile("application.yaml", "/")
        exposePort(8080)
        workingDir("/")
        entryPoint("/${nativeFileX64.name}", "-config=./application.yaml")
    }
